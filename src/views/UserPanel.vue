      ...
      <v-row align="center" justify="center" class="mx-2">
        <v-card width="600">
          <v-card-title class="white--text primary pb-2"
            >Witaj {{ user.first_name || 'Gal' }}
            {{ user.last_name || 'Anonim' }}!</v-card-title
          >
          ...
          <v-row justify="center">
            <v-btn-cap
              color="primary"
              class="mt-10"
              :to="{ name: 'userProfile', params: { id: user.profile } }"
            >
              <v-icon left>mdi-card-account-details</v-icon>
              <span>Mój profil</span>
            </v-btn-cap>
          </v-row>
          <p class="text-subtitle-1 font-weight-bold text-center mt-7">
            Opcje profilu
          </p>
        ...
        <v-card width="1000">
          ...
          <v-card-text class="py-1">
            <markdown-it-vue
              ...
              :content="
                profile.description ||
                '## <center>Brak opisu. Napisz coś o sobie :)</center>'
              "
